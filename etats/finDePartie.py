...
class FinDePartie(Etat):
    ...
    def init(self, gagner, score: int, chrono: float, niveau: str) -> None:
        ...
        self.ids = [
            fenetre.canevas.create_text(*fenetre.grille.coordonees_to_position(Vector2Int(5,21)) - Vector2(fenetre.RAYON, 1), text="REJOUER", fill=theme.text[1], font=theme.police(fenetre.RAYON*2)),
            fenetre.canevas.create_text(*fenetre.grille.coordonees_to_position(Vector2Int(19,21)) - Vector2(fenetre.RAYON, 1), text="MENU", fill=theme.text[0], font=theme.police(fenetre.RAYON*2)),
